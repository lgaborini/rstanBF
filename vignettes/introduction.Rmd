...
These need to be converted to a friendly format (matrices and lists):
```{r}

mtx_samples <- list_pop$df_pop %>% select(-source) %>% as.matrix

list_data_diff <- list(mtx = mtx_samples, idx.ref = list_samples_diff$idx_reference, idx.quest = list_samples_diff$idx_questioned)
list_data_same <- list(mtx = mtx_samples, idx.ref = list_samples_same$idx_reference, idx.quest = list_samples_same$idx_questioned)
```
...
```{r}
stanBF_obj_same <- rstanBF::compute_BF_Stan(list_data_same, 'DirDir', list_hyperpriors, data_other = NULL, n.iter, n.burnin, n.chains, n.cores)
stanBF_obj_diff <- rstanBF::compute_BF_Stan(list_data_diff, 'DirDir', list_hyperpriors, data_other = NULL, n.iter, n.burnin, n.chains, n.cores)
```
