...
```{r}
df_sources_est_ML <- fun_estimate_Dirichlet_from_samples(list_samples$df_background, use = 'MLE')
df_sources_est_ML
```
...
```{r}
df_alpha_ML_ML <- df_sources_est_ML %>% select(-source) %>% fun_estimate_Dirichlet_from_single_source(use = 'MLE')
df_alpha_ML_ML
```
